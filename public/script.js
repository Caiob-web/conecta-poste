...
//  + Postes com ícones (foto/estilo realista) + Transformadores embutidos
// =====================================================================

(() => {
  // Evita carregar/rodar duas vezes (causa redeclaração e bugs)
  if (window.__MAPA_POSTES_APP__) {
    console.warn("Mapa de Postes: script já inicializado. Ignorando nova carga.");
    return;
  }
  window.__MAPA_POSTES_APP__ = true;
  ...
  (function injectPopupCardStyles() {
    const css = `
      ...
      .mp-copy-btn{
        border:none; background:#e5f0ff; border-radius:999px;
      ...
      .mp-status-badge{
        ...
        background:#18c167; position:relative;
      }
      ...
      .mp-empresa-arrow{
        font-size:14px; color:#999; margin-left:6px;
        transition: transform .15s ease;
      }
      .mp-empresa-item.open .mp-empresa-arrow{ transform: rotate(90deg); }
      .mp-empresa-empty{
        padding:8px 10px; font-size:11px; color:#6b7280; background:#f9fafb;
      }

      /* Fotos (se existir campo no payload) */
      .mp-photos{
        margin-top:8px;
        display:grid;
  ...
  // (agora vamos USAR nos postes do mapa ✅)
  // ====================================================================
  ...
  // (estado do censo) — em variável LOCAL (não redeclara em global)
  let censoMode = false;
  let censoIds = null;
  ...
  // EXPÕE pro HTML do popup (onclick)
  window.copyBtnHandler = copyBtnHandler;
  window.toggleEmpresaExtra = toggleEmpresaExtra;
  ...
  function criarLayerPoste(p){
    ...
    // ✅ Agora é MARKER com ícone “foto” (não circleMarker)
    const layer = L.marker([p.lat, p.lon], { icon: poleIcon48(poleColorByEmpresas(qtd)) })
  ...
  // + (opcional) fotos se vierem no payload
  // ---------------------------------------------------------------------
  function montarPopupModeloCard(p) {
    ...
    // ✅ Fotos opcionais (se existir no payload)
    const fotos = [];
    const pick = (label, url) => {
      if (!url) return;
      const u = String(url).trim();
      if (!u) return;
      fotos.push({ label, url: u });
    };

    // tente vários nomes comuns (não quebra se não existir)
    pick("Antes", p.foto_antes || p.fotoAntes || p.foto_pre || p.foto_pre_obra);
    pick("Depois", p.foto_apos || p.fotoApos || p.foto_depois || p.foto_pos || p.foto_pos_obra);

    // se vier array genérico
    if (Array.isArray(p.fotos)) {
      p.fotos.slice(0, 4).forEach((u, i) => pick(`Foto ${i+1}`, u));
    }
      ...
      ? `<div class="mp-photos">
          ${fotos.slice(0,2).map(f => `
            <div class="mp-photo">
              ...
              <div class="cap">${escapeHtml(f.label)}</div>
            </div>
          `).join("")}
        </div>`
      : "";
  ...
  // ---------------------------------------------------------------------
  // Interações / filtros (expostas pro HTML via onclick)
  // ---------------------------------------------------------------------
  function buscarID() {
  ...
  /* ====================================================================
     TRANSFORMADORES — Camada própria (toggle) — SEM /assets (data-uri)
  ==================================================================== */
  ...
  function parseLatLon(obj) {
    ...
    if (obj.lat != null && obj.lon != null) {
      const lat = tryNum(obj.lat), lon = tryNum(obj.lon);
      if (lat != null && lon != null) return { lat, lon };
    }
    if (obj.latitude != null && obj.longitude != null) {
      const lat = tryNum(obj.latitude), lon = tryNum(obj.longitude);
      if (lat != null && lon != null) return { lat, lon };
    }
    const c = obj.coordenadas || obj.coordenada || obj.coord;
    if (c) {
      const [lat, lon] = String(c).split(/,\s*/).map(Number);
  ...
  function popupTransformadorHTML(t) {
    const id = escapeHtml(t.id ?? t.codigo ?? t.nome ?? "—");
    const tipo = escapeHtml(t.tipo ?? t.modelo ?? "Transformador");
    const pot = escapeHtml(t.potencia ?? t.kva ?? "—");
    const poste = escapeHtml(t.poste ?? t.id_poste ?? "—");
    return `
      <div class="mp-card">
        <div class="mp-header">
          <div class="mp-header-title">Transformador</div>
          <div class="mp-header-sub">ID: ${id}</div>
        </div>
        <div class="mp-local">
          <div class="mp-local-principal">${tipo}</div>
          <div class="mp-local-secundario">Potência: <b>${pot}</b> · Poste: <b>${poste}</b></div>
        </div>
      </div>
    `.trim();
  }
  ...
  async function carregarTransformadores() {
    ...
    try {
      ...
      (Array.isArray(data) ? data : []).forEach((t) => {
        ...
        const mk = L.marker([ll.lat, ll.lon], {
          icon: ICON_TRANSFORMADOR,
          pane: "transformadores",
        });

        mk.bindTooltip(`Transformador`, { direction: "top", sticky: true });
        mk.on("click", (e) => {
          if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent);
          const html = popupTransformadorHTML(t);
          mainPopup.setLatLng([ll.lat, ll.lon]).setContent(html);
          if (!map.hasLayer(mainPopup)) mainPopup.addTo(map);
          popupPinned = true;
          lastPopup = { lat: ll.lat, lon: ll.lon, html };
        });
      ...
      console.warn("Falha ao carregar transformadores:", e);
      // não trava o app — só desliga a camada
      const chk = document.getElementById("chkTransformadores");
      if (chk) {
        chk.checked = false;
        chk.disabled = true;
        chk.title = "Endpoint /api/transformadores não disponível";
      }
  ...
  /* --------------------------------------------------------------------
     === Indicadores (BI) — (mantido igual ao seu)
  -------------------------------------------------------------------- */
