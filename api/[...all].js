// api/[...all].js — Express catch-all no Vercel (sem vercel.json)
...
app.get("/api/postes", async (req, res) => {
  try {
    ...
    if (!(await hasRelation("public.dados_poste")) && !(await hasRelation("dados_poste")))
...
app.get("/api/ov/_debug", async (_req, res) => {
  try {
    const exists = await Promise.all(
      TABLE_OV_CANDIDATES.map(async n => ({ name: n, exists: await hasRelation(n) }))
    );
    let mapping = null, error = null;
    try {
      mapping = await getOvColumns();
    } catch (e) {
      error = e.message;
    }
    res.json({ candidates: exists, mapping, error });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
