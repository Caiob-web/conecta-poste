// api/[[...path]].js — todas as rotas /api/* (Express dentro de função serverless)
...
app.post("/api/postes/report", async (req, res) => {
  try {
    ...
    const table = (await hasRelation("indicadores_v_ocupacao"))
      ? "indicadores_v_ocupacao" : "dados_poste";
...
// RAW esperado pelo front (e aliases antigos)
async function handlerOVRaw(_req, res) {
  try {
    const tbl = await resolveOvTable();
    const sql = `
      WITH base AS (${baseOvSql(tbl)})
      SELECT empresa, municipio, ordem_venda AS ordem, status, postes, data_envio_carta AS data
      FROM base
    `;
    const r = await pool.query(sql);
    res.setHeader("Cache-Control", "no-store");
    res.json(r.rows ?? []);
  } catch (e) { console.error("Erro /api/ov:", e); res.status(500).json({ error: "Falha ao consultar OV" }); }
}
...
app.get("/api/ov/kpis", async (req, res) => {
  try {
    ...
    res.json({
      ...
      status: status.rows, topEmpresas: empresas.rows, porMunicipio: municipios.rows,
    });
...
app.get("/api/ov/list", async (req, res) => {
  try {
    ...
    const sql = `
      WITH base AS (${baseOvSql(tbl)})
      SELECT ordem_venda, empresa, municipio, status,
             COALESCE(postes,0)::int AS postes, data_envio_carta, carta
      FROM base
      ${whereSql}
...
/* ---------- Stubs para chamadas legadas (evitar 404 no console) ---- */
app.get("/api/indicadores", (_req, res) => res.json([]));
app.get("/api/ocupacoes_postes", (_req, res) => res.json([]));
app.get("/api/ocupacoes_postes_sumario", (_req, res) => res.json([]));

/* ---------- Healthcheck -------------------------------------------- */
app.get("/api/health", (_req, res) => res.json({ ok: true }));

// Exporta o Express como handler serverless da Vercel
export default app;
